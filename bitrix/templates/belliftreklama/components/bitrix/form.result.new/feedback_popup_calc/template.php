...
$formH = str_replace('<form', '<form class="contact_feedback" id="order_feedback_popup" name="order_feedback_popup"', $formH);
?>
<?= $formH ?>
...
<div class="left">
    <? foreach ($arResult["QUESTIONS"] as $FIELD_SID => $arQuestion): ?>
        ...
        elseif ($arQuestion['STRUCTURE'][0]['FIELD_TYPE'] == 'textarea'):
            ...
            $textarea = str_replace('<textarea', '<textarea id="order_input_textarea"', $textarea);
        else: ?>
            <div class="row">
                ...
                switch($FIELD_SID){
                    case 'PHONE':
                        $input = str_replace('<input', '<input id="order_input_phone"', $input);
                        break;
                    case 'NAME':
                        $input = str_replace('<input', '<input id="order_name_input"', $input);
                        break;
                    case 'EMAIL':
                        $input = str_replace('<input', '<input id="order_input_email"', $input);
                        break;
                }
                echo $input;
                ?>
